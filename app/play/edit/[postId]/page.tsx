  ...
  if (!post || !isOwner) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-6 text-center gap-3">
        ...
        <a href="/calendar" className="px-6 py-3 rounded-btn bg-accent text-white font-bold text-sm mt-2">Back to Calendar</a>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-screen px-4 pt-16 pb-8">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="flex-1">
          <h1 className="text-xl font-extrabold">Edit Match Post</h1>
          <p className="text-xs text-text-secondary mt-0.5">{post.team_name}</p>
        </div>
        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-full flex-shrink-0 ${post.status === "matched" ? "bg-accent/10 text-accent-ink border border-accent/30" : "bg-surface-2 border border-border text-text-secondary"}`}>

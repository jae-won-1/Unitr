  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data: bks } = await supabase
        .from("pitch_bookings")
        .select("id, match_date, start_time, player_count, per_player_pence, status, booked_by")
        .eq("pitch_id", p.id)
      ...
      const enriched = await Promise.all((bks ?? []).map(async (b) => {
        ...
        return { ...b, booker_name: (prof as { full_name: string } | null)?.full_name ?? "Unknown" };
      }));
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="text-lg font-bold truncate">{pitch.name}</h1>
            {pitch.is_verified && (
          ...
          <p className="text-xs text-text-secondary truncate">{pitch.address}</p>
        </div>
      </div>
            ...
            {upcoming.map((b) => (
              <div key={b.id} className="bg-surface-2 border border-border rounded-2xl px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-secondary truncate">{b.match_date} · {b.player_count} players · £{((b.per_player_pence * b.player_count) / 100).toFixed(0)}</p>
                </div>

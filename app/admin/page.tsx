    ...
    async function load() {
      ...
      if (rows.length) {
        const { data: joined } = await supabase.from("open_match_teams")
          .select("open_match_id").in("open_match_id", rows.map((r) => r.id));
        for (const j of joined ?? []) counts.set(j.open_match_id as string, (counts.get(j.open_match_id as string) ?? 0) + 1);
      }
      setEvents(rows.map((r) => ({ ...r, joined: counts.get(r.id) ?? 0 } as HostedEvent)));
      setLoading(false);
    }
    load();
  }, [user]);

...
// platform fee by setting UNITER_FEE_RATE back to 0.05 (or whatever is agreed)
...
export const UNITER_FEE_RATE = 0;

/** False while Uniter takes no cut — hide fee lines entirely rather than showing £0.00. */
export const UNITER_FEE_ENABLED = UNITER_FEE_RATE > 0;

/** The rate as a label, e.g. "5%". */
export const UNITER_FEE_LABEL = `${Number((UNITER_FEE_RATE * 100).toFixed(2))}%`;

/** The fee charged on top of `basePence`. */
export function feeOn(basePence: number): number {
  return Math.round(basePence * UNITER_FEE_RATE);
}
...
export function feeWithin(totalPence: number): number {
  return Math.round(totalPence - totalPence / (1 + UNITER_FEE_RATE));
}
